...
"""
This command is intented to produce a Dash app (i.e html interface) from an OLOGRAM csv output.
"""
import argparse
import os
import sys
...
__notes__ = """
-- This is an alternative to the default pdf output of OLOGRAM and OLOGRAM-MODL.
-- Currently the command produces barplots and volcano plot with some tweaking menus.
-- The -\-output is not functional at the moment. 
...
def make_parser():
    ...
    parser_grp.add_argument('-o', '--outputfile',
                            help="Output file (not functional at the moment).",
                            default=sys.stdout,
                            metavar="GTF",
...
def html_diagram(inputfile=None,
    ...
    ####################################################################################################
    # Load the table using the dedicated functions
    ####################################################################################################

    dmm = loading_and_preparing_ologram_table_barplot(inputfile.name)
    df_volc = loading_and_preparing_ologram_table_volcano(inputfile.name)
    ...
    original_table_layout = html.Div(children=[
        dbc.Card(
            dbc.CardBody([
                dbc.Table.from_dataframe(pd.read_csv(inputfile,
                                                     sep="\t", header=0),
                                         striped=True,
                                         bordered=True,
                                         hover=True,
    ...
    app.run_server(host='0.0.0.0',debug=True, port=8071)
...
if __name__ == '__main__':
    ...
    test = """
       ...
       # html_diagram: just a placeholder at the moment
       @test "html_diagram" {
        result=`gtftk html_diagram -h > /dev/null`
         [ "$result" = "" ]
       }  
    """
    ...
    # coordinates, annotation, sequences, coverage, ologram
    # and miscellaneous.

    cmd = CmdObject(name="html_diagram",
                    message="This command is intented to produce a Dash app (i.e html interface) from an OLOGRAM csv output.",
                    parser=make_parser(),
